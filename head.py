    ...
    def forward(self, x):
        ...
        for l, feature in enumerate(x):
            ...
            bbox_reg.append(torch.exp(self.scales[l](
                self.bbox_pred(self.bbox_tower(feature))  #回归盒子的得分 bbox有不同si调整bbox大小
            )))
...
#计算特征金字塔中每一层的特征图中每个像素对应到原始输入图像中的位置，该对应位置一般在感受野中心附近.
    def compute_locations_per_level(self, h, w, stride, device):
        ...
        )#以8为间隔，一个列表，从0-8*（W-1）　即[0,8,16,...,1016]
        shifts_y = torch.arange(
            0, h * stride, step=stride,
            dtype=torch.float32, device=device
        )#以8为间隔，一个列表，从0-8*（H-1）　即[0,8,16,...,792]
        ...
        return locations #得到该layer在原图感受野中心的坐标
